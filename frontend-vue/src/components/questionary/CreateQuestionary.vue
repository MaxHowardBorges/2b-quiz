<template>
  <v-sheet class="mt-20px align-center" v-if="question">
    ...
    <v-sheet
      ...
      <div class="answers">
        <v-radio-group v-model="correct">
          <v-radio :id="'correct-answer-1'" :value="0">{{ $t('questionnary.True') }}</v-radio>
          <v-radio :id="'correct-answer-2'" :value="1">{{ $t('questionnary.False') }}</v-radio>
        </v-radio-group>
      </div>
    </v-sheet>
    ...
    <v-sheet
      ...
      <v-btn color="primary" @click="addAnswer" class="mr-2">
        {{ $t('questionnary.addAnswer') }}
      </v-btn>
      <v-btn color="error" @click="removeAnswer()">
        {{ $t('questionnary.deleteAnswer') }}</v-btn>
    </v-sheet>
  </v-sheet>
</template>
